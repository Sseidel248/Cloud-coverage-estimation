...
class TesCloudCoverData(unittest.TestCase):
    def test__init__(self):
        test_obj = CloudCoverData(tc.EXISTING_TESTFILE_D2)
    ...
    def test_get_info_str(self):
        ...
        test_obj_eu = CloudCoverData(tc.EXISTING_TESTFILE_EU)
...
class TestInvalidCloudCoverage(unittest.TestCase):
    def test__init__(self):
        inval_cc = InvalidCloudCoverages("Hallo Welt")
    ...
    def test_show_warning_empty(self):
        inval_cc = InvalidCloudCoverages("")
    ...
    def test_show_warning(self):
        inval_cc = InvalidCloudCoverages("")
        cc = CloudCoverData(tc.EXISTING_TESTFILE_D2)
        inval_cc.add(cc)
        with patch("Lib.ColoredPrint.show_warning") as mock_show_warning:
            inval_cc.show_warnings()
...
class TestCloudCoverDatas(unittest.TestCase):
    def test__init__(self):
        ccs = CloudCoverDatas(mConst.MODEL_ICON_D2)
    ...
    def test_add_invalid(self):
        ...
        self.assertFalse(ccs.add("", data=tc.EXISTING_TESTFILE_D2))
        ...
        cc_none_datetime = CloudCoverData(tc.MODEL_FILE_WRONG_MODEL)
    ...
    def test_add(self):
        ccs = CloudCoverDatas(mConst.MODEL_ICON_D2)  # forecast 60min -> 1h
    ...
    def test_lat_in_range(self):
        ccs_d2 = CloudCoverDatas(mConst.MODEL_ICON_D2)
        ...
        ccs_eu = CloudCoverDatas(mConst.MODEL_ICON_EU)
...
class TestCloudCoverageReaderFunctions(unittest.TestCase):
    def test_get_warn_str(self):
        ...
        cc_d2 = CloudCoverData(tc.EXISTING_TESTFILE_D2)
        ...
        cc_eu = CloudCoverData(tc.EXISTING_TESTFILE_EU)
    ...
    def test_init_cloud_cover_data(self):
        ccs = init_cloud_cover_data(tc.TEST_DIR_MODEL_D2, mConst.MODEL_ICON_D2)
        self.assertEqual(ccs.stderr, "")
    ...
    def test_init_cloud_cover_data_invalid(self):
        ccs_path_not_exist = init_cloud_cover_data(tc.TEST_DIR_NOT_EXIST, "Hans Wurst")
        self.assertEqual(ccs_path_not_exist.stderr, f"{ewConst.ERROR_WRONG_MODEL}: Wrong model")

        ccs_wrong_param = init_cloud_cover_data(tc.TEST_DIR_NOT_EXIST, mConst.MODEL_ICON_D2)
        self.assertEqual(ccs_wrong_param.stderr, f"{ewConst.ERROR_PATH_NOT_EXIST}: Path does not exist")

        ccs_empty = init_cloud_cover_data(tc.EMPTY_TEST_DIR, mConst.MODEL_ICON_D2)
        self.assertEqual(ccs_empty.stderr, f"{ewConst.ERROR_NO_FILES_FOUND}: No grib2 files found")

        ccs_invalid = init_cloud_cover_data(tc.TEST_DIR_MODEL_D2_INVALID_FILES, mConst.MODEL_ICON_D2)
        self.assertEqual(ccs_invalid.stderr, f"{ewConst.ERROR_GRIB2_FILES_INCOMPLETE}: "
                                             f"There are no files with correct data")

        ccs_mix = init_cloud_cover_data(tc.TEST_DIR_MODEL_D2_MIX_FILES, mConst.MODEL_ICON_D2)
