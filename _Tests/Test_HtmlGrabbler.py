...
class TestHtmlGrabbler(unittest.TestCase):
    def tearDown(self):
        self.assertTrue(os.path.exists(tc.DOWNLOAD_DIR))
        if os.path.exists(f"{tc.DOWNLOAD_DIR}/file.txt"):
            os.remove(f"{tc.DOWNLOAD_DIR}/file.txt")
    ...
    def test_get_html_links_as_list(self, mock_get):
        ...
        self.assertEqual(links, ["link1.html", "link2.html"])
    ...
    def test_download_data(self, mock_file, mock_get):
        ...
        download_data([DownloadData(url="http://example.com/file.txt", file="file.txt", path=tc.DOWNLOAD_DIR)])
        mock_file.assert_called_with(f"{tc.DOWNLOAD_DIR}\\file.txt", 'wb')
