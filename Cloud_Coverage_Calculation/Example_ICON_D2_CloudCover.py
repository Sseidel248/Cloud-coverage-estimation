...
from Lib.CloudCoverageReader import init_cloud_cover_data

res = init_cloud_cover_data("..\\DWD_Downloader\\WeatherData\\ICON_D2", mConst.MODEL_ICON_D2)
...
if len(res.cloud_cover_files) > 0:
    ...
    lat = 52.0613
    lon = 13.4997
    print(f"Cloud Cover at {lat}, {lon} [lat, lon] for {str(date_time)}: {res.get_cloud_cover(date_time, lat, lon)}%")
